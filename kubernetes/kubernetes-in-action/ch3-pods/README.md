...
<!--ts-->
  - [3.1 Introducing Pods](#3.1-Introducing-Pods)
    - [왜 쿠버네티스는 컨테이너를 직접 사용하지 않고 파드라는 개념을 만들었을까?](#왜-쿠버네티스는-컨테이너를-직접-사용하지-않고-파드라는-개념을-만들었을까?)
    - [파드에서 컨테이너를 격리하는 수준](#파드에서-컨테이너를-격리하는-수준)
    - [파드 간 네트워크 구성](#파드-간-네트워크-구성)
    - [컨테이너 그룹화](#컨테이너-그룹화)
    - [](#)
  - [3.2 Creating Pods from YAML Descriptor](#3.2-Creating-Pods-from-YAML-Descriptor)
    - [Descriptor를 통해 파드 생성](#Descriptor를-통해-파드-생성)
  - [3.3 Organizing with Labels](#3.3-Organizing-with-Labels)
    - [라벨과 함께 파드 생성](#라벨과-함께-파드-생성)
    - [기존 파드의 라벨 수정](#기존-파드의-라벨-수정)
  - [3.4 파드의 노드 예약 제한](#3.4-파드의-노드-예약-제한)
    - [노드에 분류를 위한 라벨 설정](#노드에-분류를-위한-라벨-설정)
    - [특정 노드에 파드 예약](#특정-노드에-파드-예약)
  - [3.5 Annotating](#3.5-Annotating)
    - [주석 조회](#주석-조회)
    - [주석 추가 및 수정](#주석-추가-및-수정)
  - [3.6 Using Namespace](#3.6-Using-Namespace)
    - [네임스페이스 및 해당 포드 출력](#네임스페이스-및-해당-포드-출력)
    - [네임스페이스 생성](#네임스페이스-생성)
    - [네임스페이스에서 리소스 관리](#네임스페이스에서-리소스-관리)
    - [네임스페이스를 통한 격리 수준](#네임스페이스를-통한-격리-수준)
  - [3.7 Stopping and Removing Pods](#3.7-Stopping-and-Removing-Pods)
...
## 3.2 Creating Pods from YAML Descriptor
