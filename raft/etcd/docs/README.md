...
<!--ts-->
  ...
  - [etcd Raft Readme](#etcd-Raft-Readme)
	...
	- [Usage](#Usage)
		...
		- [2. Node.Ready() 채널을 읽어서 스토리지를 업데이트하거나 네트워크를 통해 다른 노드로 메시지 전송](#2.-Node.Ready()-채널을-읽어서-스토리지를-업데이트하거나-네트워크를-통해-다른-노드로-메시지-전송)
		- [3. 주기적으로 Node.Tick()을 호출해서 HeartbeatTimeout, ElectionTimeout 발생시키기](#3.-주기적으로-Node.Tick()을-호출해서-HeartbeatTimeout,-ElectionTimeout-발생시키기)
		- [4. raft 모듈 내부로 필요한 메시지를 전달](#4.-raft-모듈-내부로-필요한-메시지를-전달)
  - [네트워크, 스토리지 계층과 raftpb.Message](#네트워크,-스토리지-계층과-raftpb.Message)
	- [raftpb.Message](#raftpb.Message)
	- [raft.Node에서 네트워크 게층으로](#raft.Node에서-네트워크-게층으로)
	- [네트워크 계층에서 raft.Node로](#네트워크-계층에서-raft.Node로)
  - [raft.Node가 raftpb.Message를 처리하는 방법](#raft.Node가-raftpb.Message를-처리하는-방법)
	- [Step 함수](#Step-함수)
  - [raft.Node가 raftpb.Message를 Application에게 전달하는 방법](#raft.Node가-raftpb.Message를-Application에게-전달하는-방법)
  ...
  - [raft.Node에서 채널 이벤트 기반으로 오케스트레이션 하기](#raft-Node에서-채널-이벤트-기반으로-오케스트레이션-하기)
	- [이벤트 루프](#이벤트-루프)
